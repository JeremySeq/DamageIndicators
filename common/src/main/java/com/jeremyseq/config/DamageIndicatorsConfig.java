...
public class DamageIndicatorsConfig {
    ...
    public int hitmarkerDuration = 5;
    public int hitmarkerSize = 10;
    public float hitmarkerThickness = 1.75f;
    public int hitmarkerGap = 3;
    ...
    public static void load() {
        if (Files.exists(CONFIG_PATH)) {
            ...
            INSTANCE.enableHitmarker = config.getOrElse("hitmarker.enable", defaults.enableHitmarker);
            INSTANCE.hitmarkerDuration = config.getIntOrElse("hitmarker.duration", defaults.hitmarkerDuration);
            INSTANCE.hitmarkerSize = config.getIntOrElse("hitmarker.size", defaults.hitmarkerSize);
            INSTANCE.hitmarkerThickness = config.<Number>getOrElse("hitmarker.thickness", defaults.hitmarkerThickness).floatValue();
            INSTANCE.hitmarkerGap = config.getIntOrElse("hitmarker.gap", defaults.hitmarkerGap);
    ...
    private static void writeWithComments(CommentedFileConfig config) {
        ...
        config.setComment("hitmarker", " a brief X shown when you hit another entity");

        config.set("hitmarker.enable", INSTANCE.enableHitmarker);
        config.setComment("hitmarker.enable", " enables the hit marker");

        config.set("hitmarker.duration", INSTANCE.hitmarkerDuration);
        config.setComment("hitmarker.duration", " time for the hit marker to show (in ticks)");

        config.set("hitmarker.size", INSTANCE.hitmarkerSize);
        config.setComment("hitmarker.size", " length of the hit marker lines");

        config.set("hitmarker.thickness", INSTANCE.hitmarkerThickness);
        config.setComment("hitmarker.thickness", " thickness of the hit marker lines");

        config.set("hitmarker.gap", INSTANCE.hitmarkerGap);
        config.setComment("hitmarker.gap", " gap of the hitmarker lines");
